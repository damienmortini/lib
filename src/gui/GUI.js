...
export default class GUI extends HTMLElement {
  ...
  set serverUrl(value) {
    ...
    const sendQueue = () => {
      ...
      for (const data of this._webSocketQueue) {
    ...
    this._onWebSocketMessage = (e) => {
      ...
      const input = this._inputs.get(data.uid);
      if (input._client) {
  ...
  add({
    ...
    const groupKey = normalizeString(group);
    const uid = groupKey ? `${groupKey}/${idKey}` : idKey;
    ...
    const input = document.createElement('dlib-guiinput');
