...
for (const [key, value] of POSE_ROTATIONS) {
...
class Hand {
  ...
  update(handData, pointablesData) {
    ...
    // arm
    const armBone = this.bones.get('arm');
    ...
    // wrist
    const wristBone = this.bones.get('wrist');
    ...
    // fingers
    for (const pointableData of pointablesData) {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        const boneName = `${HandTracker.FINGER_NAMES[pointableData.type]}${i}`;
        const bone = this.bones.get(boneName);
        const basis = pointableData.bases[dataBoneIndex];
