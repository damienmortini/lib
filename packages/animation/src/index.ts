...
/**
 * A keyframe is either the destination value (the start value is read from the
 * target when the animation begins) or an explicit `[from, to]` pair.
 */
export type Keyframe = number | [from: number, to: number];

export type Keyframes<Target> = {
  [Key in keyof Target as Target[Key] extends number ? Key : never]?: Keyframe;
};
...
export const animate = <Target extends Record<string, number>>(
  ...
  const computedKeyframes = new Map(Object.entries(keyframes) as [string, Keyframe][]);
  ...
  for (const [key, value] of computedKeyframes) {
    resolvedKeyframes.set(key, value instanceof Array ? value : [target[key], value]);
  }
