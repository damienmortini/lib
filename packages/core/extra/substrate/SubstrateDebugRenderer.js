...
export default class SubstrateDebugRenderer {
  ...
  update() {
    if (this.edgesDebug) {
      ...
      for (let i = 0; i < this.substrateSystem.data.length; i++) {
        const id = this.substrateSystem.data[i];
        if (id !== 0) {
          const debugColor = this.getDebugColor(id, this._cachedVector3);
  ...
  drawEdges() {
    for (let i = 0; i < this.substrateSystem.edges.length; i++) {
      const edge = this.substrateSystem.edges[i];

      const debugColor = this.getDebugColor(edge.id, this._cachedVector3);
      ...
      p1 = edge.getPointFromRatio(0.6);
      p2 = edge.next.getPointFromRatio(0.4);

      this.context.moveTo(p1.x, p1.y);
      this.context.quadraticCurveTo(edge.next.a.x, edge.next.a.y, p2.x, p2.y);

      p1 = edge.twin.getPointFromRatio(0.6);
      p2 = edge.twin.next.getPointFromRatio(0.4);
  ...
  drawPolygons() {
    for (let i = 0; i < this.substrateSystem.polygons.length; i++) {
      const polygon = this.substrateSystem.polygons[i];

      const debugColor = this.getDebugColor(polygon.id, this._cachedVector3);
  ...
  getDebugColor(id, outVector = new Vector3()) {
    const moduloId = id % 6;
    const r = (moduloId === 1 || moduloId === 4 || moduloId === 6) ? 255 : 0;
    const g = (moduloId === 2 || moduloId === 4 || moduloId === 5) ? 220 : 0;
    const b = (moduloId === 3 || moduloId === 5 || moduloId === 6) ? 255 : 0;
