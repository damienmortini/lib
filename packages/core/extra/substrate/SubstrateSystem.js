...
export default class SubstrateSystem {
  ...
  addPolygon(vertices) {
    const polygon = new SubstratePolygon(vertices);
  ...
  update() {
    for (let i = 0; i < this.speed; i++) {
      for (let i = 0; i < this.edges.length; i++) {
        ...
        const position = Math.floor(edge.boid.position.x) + this.width * Math.floor(edge.boid.position.y);

        const pixelId = this.data[position];
        ...
        if (this.edges.length < this.maxEdges && Math.random() < this.spawnProbabilityRatio) {
          ...
          const spawnEdge = this.spawnEdge(edge.boid.position.x, edge.boid.position.y, velocityAngle, edge.boid.life);
  ...
  splitEdgeWithEdge(splittedEdge, edge) {
    const angle = splittedEdge.boid.velocity.angleTo(edge.boid.velocity);
    ...
    const newEdge = this.spawnEdge(this._sweepBoid.position.x, this._sweepBoid.position.y);
    ...
    const collided = edge.boid.isDead;
    ...
    while (sweepSecurityMargin < 3) {
      for (let i = -4; i < 5; i++) {
        const sweepPosition = Math.floor(this._sweepBoid.position.x + this._sweepBoid.velocity.y * i * 0.33) + this.width * Math.floor(this._sweepBoid.position.y - this._sweepBoid.velocity.x * i * 0.33);
  ...
  polygonCheck(startEdge) {
    ...
    const vertices = [new Vector2(edge.a.x, edge.a.y)];
    for (let i = 0; i < edgesNumber; i++) {
      if (Math.abs(edge.angle - edge.next.angle) > 0.1) {
