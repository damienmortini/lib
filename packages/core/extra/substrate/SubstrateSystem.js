...
export default class SubstrateSystem {
  ...
  splitEdgeWithEdge(splittedEdge, edge) {
    ...
    while (sweepSecurityMargin < 3) {
      for (let i = -4; i < 5; i++) {
        const sweepPosition = Math.floor(this._sweepBoid.position.x + this._sweepBoid.velocity.y * i * .33) + this.width * Math.floor(this._sweepBoid.position.y - this._sweepBoid.velocity.x * i * .33);
