...
export default class SoundMatrix extends Map {
  ...
  set(sound, array, { clip } = {}) {
    ...
    const soundArray = this.get(sound);
  ...
  _update() {
    ...
    for (const [sound, array] of this) {
      if (this._clips.has(sound)) {
        ...
        const clone = this._clones.get(sound)[this.beat % this._clones.get(sound).length];
