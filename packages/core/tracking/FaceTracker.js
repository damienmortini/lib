...
export default class FaceTracker {
  constructor(input, { width = input.width, height = input.height } = {}) {
    ...
    // Compute indices
    const cells = pModel.path.vertices.concat([[44, 61, 56], [61, 60, 56], [56, 60, 57], [60, 59, 57], [57, 59, 58], [58, 59, 50]]);
    ...
    for (let i = 0; i < pModel.shapeModel.meanShape.length; i++) {
      const position = pModel.shapeModel.meanShape[i];
      // Normalize values
      const x = (position[0] - 24.381567267455893) / 65.00000000000003 * 2 - 1;
      const y = -(position[1] - 25.17298033285374) / 64.61033114464396 * 2 + 1;
  ...
  get positions() {
    ...
    if (positions && this._lastComputedFrame !== this._currentFrame) {
      for (let i = 0; i < positions.length; i++) {
        ...
        this._positions[i * 2] = -(position[0] - this._inputCanvas.width * 0.5) / this._inputCanvas.width * 2;
        this._positions[i * 2 + 1] = -(position[1] - this._inputCanvas.height * 0.5) / this._inputCanvas.height * 2;
      }
    }
  ...
  get mapTexture() {
    ...
    for (let i = 0; i < this._indices.length; i += 3) {
      const id0 = this._indices[i] * 2;
      const id1 = this._indices[i + 1] * 2;
      const id2 = this._indices[i + 2] * 2;
      const x0 = (this.initialPositions[id0] * 0.5 + 0.5) * this._mapCanvas.width;
      const y0 = (-this.initialPositions[id0 + 1] * 0.5 + 0.5) * this._mapCanvas.height;
      const x1 = (this.initialPositions[id1] * 0.5 + 0.5) * this._mapCanvas.width;
      const y1 = (-this.initialPositions[id1 + 1] * 0.5 + 0.5) * this._mapCanvas.height;
      const x2 = (this.initialPositions[id2] * 0.5 + 0.5) * this._mapCanvas.width;
      const y2 = (-this.initialPositions[id2 + 1] * 0.5 + 0.5) * this._mapCanvas.height;
      const u0 = (this.positions[id0] * 0.5 + 0.5) * this._inputCanvas.width;
      const v0 = (-this.positions[id0 + 1] * 0.5 + 0.5) * this._inputCanvas.height;
      const u1 = (this.positions[id1] * 0.5 + 0.5) * this._inputCanvas.width;
      const v1 = (-this.positions[id1 + 1] * 0.5 + 0.5) * this._inputCanvas.height;
      const u2 = (this.positions[id2] * 0.5 + 0.5) * this._inputCanvas.width;
      const v2 = (-this.positions[id2 + 1] * 0.5 + 0.5) * this._inputCanvas.height;
