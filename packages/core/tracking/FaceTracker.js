...
export default class FaceTracker {
  ...
  get mapTexture() {
    ...
    for (let i = 0; i < this._indices.length; i += 3) {
      ...
      const x0 = (this.initialPositions[id0] * .5 + .5) * this._mapCanvas.width;
      const y0 = (-this.initialPositions[id0 + 1] * .5 + .5) * this._mapCanvas.height;
      const x1 = (this.initialPositions[id1] * .5 + .5) * this._mapCanvas.width;
      const y1 = (-this.initialPositions[id1 + 1] * .5 + .5) * this._mapCanvas.height;
      const x2 = (this.initialPositions[id2] * .5 + .5) * this._mapCanvas.width;
      const y2 = (-this.initialPositions[id2 + 1] * .5 + .5) * this._mapCanvas.height;
      const u0 = (this.positions[id0] * .5 + .5) * this._inputCanvas.width;
      const v0 = (-this.positions[id0 + 1] * .5 + .5) * this._inputCanvas.height;
      const u1 = (this.positions[id1] * .5 + .5) * this._inputCanvas.width;
      const v1 = (-this.positions[id1 + 1] * .5 + .5) * this._inputCanvas.height;
      const u2 = (this.positions[id2] * .5 + .5) * this._inputCanvas.width;
      const v2 = (-this.positions[id2 + 1] * .5 + .5) * this._inputCanvas.height;
