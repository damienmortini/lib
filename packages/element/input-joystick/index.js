...
export default class InputJoystickElement extends HTMLElement {
  ...
  _update() {
    ...
    this._lerpedPosition.x += (this._position.x - this._lerpedPosition.x) * (this._pointerDowned ? 0.5 : 0.1);
    this._lerpedPosition.y += (this._position.y - this._lerpedPosition.y) * (this._pointerDowned ? 0.5 : 0.1);

    if (!this._pointerDowned) {
      if (this._lerpedPosition.size < 0.001) {
        this._lerpedPosition.set(0, 0);
        cancelAnimationFrame(this._animationFrameID);
      }
    }

    this._joystick.style.transform = `translate(${this._lerpedPosition.x * this._boundingClientRect.width * 0.5}px, ${this._lerpedPosition.y * this._boundingClientRect.height * 0.5}px)`;
